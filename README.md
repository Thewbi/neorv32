...
Another use-case where UART can be used (and is used very often) is Human-Machine interaction between a computing system and a human. On Microsoft Windows operating systems, UART connections are abstracted via the concept of COM ports! On linux the UART is abstracted behind a file in the devices folder. Here is a screenshot of the Microsoft Windows Device Manager:

![MicrosoftWindowsDeviceManager.png](res/images/MicrosoftWindowsDeviceManager.png)

As soon as windows detects a serial connection such as a serial connection provided by a UART, it will automatically create COM-port on the fly, dynamically. You never know which number Microsoft Windows will assign to a new serial connection that it has not seen before. Once detected, windows uses the same id for the same serial connection meaning, when you unplug and replug the USB-cable that transmitts the serial connection, windows will detect the connection again and reuse the same id for the COM port.

The approach I use is to first open the Device Manager and search for the Ports (COM & LPT). If the node is not in the list, everything is fine. It the node is part of the list, unfold the node and check all existing COM ports and memorize the ids. Then plug in the cable to your new device. Windows will detect the serial connection, dynamically created the COM port and the Device Manager will reload, displaying the new COM Port. If you have remembered the old list, you are now able to identify the id of the new COM port added. Use this id in a terminal emulator such as YAT to establish a connection using the baudrate of 19200 since this is the baudrate that is configured in the NEORV32 source code.

In the case of the NEORV32 CPU, the CPU provides a UART for a human to connect their Laptop/Desktop PC to and to start a terminal emulator that connects to a COM port on windows.
