...
- [Measuring Execution Times](#measuring-execution-times)
...
In order to facilitate uploading files, I have written an simple UART - Uploader: https://github.com/Thewbi/neorv_bootloader_app_uploader. There is an executable file under the releases section: https://github.com/user-attachments/files/24492554/upload_000.zip. In order for the uploader to be able to talk to the bootloader, make sure that the COM-port of serial port file in case of linux is not used by another program as you start the uploader. Only a single application can use a COM-port.
...
UART is a general construct. As stated it is used to transmit bytes between components. For example at work we use a bluetooth module which is connected to a microcontroller via a UART. Received bytes from bluetooth enter the bluetooth module, are handled by the Bluetooth Stack running on the module and the final payload, wrapped inside a blutooth message is then sent to the Microcontroller via the UART. The C-Application running on the Microcontroller reads the payload from the UART and processes the message.

Another use-case where UART can be used (and is used very often) is Human-Machine interaction between a computing system and a human. The way this works is that one of the components is the computing system (In our case, the NEORV32 CPU) and the other component is a terminal emulator started by a human so that commands can be sent to the computer system and human readable responses can be read on the terminal screen.

On Microsoft Windows operating systems, UART connections are abstracted via the concept of COM ports! On linux the UART is abstracted behind a file in the devices folder. Here is a screenshot of the Microsoft Windows Device Manager:

![MicrosoftWindowsDeviceManager.png](res/images/MicrosoftWindowsDeviceManager.png)

As soon as windows detects a serial connection such as a serial connection provided by a UART, it will automatically create COM-port on the fly, dynamically. You never know which number Microsoft Windows will assign to a new serial connection that it has not seen before. Using the advanced properties of the COM-node in the device manager it is possible to change the ID of the COM-Port to a user-choosen ID if you need that feature! Once detected, windows uses the same id for the same serial connection, meaning when you unplug and replug the USB-cable that transmitts the serial connection, windows will detect the connection again and reuse the same id for the COM port. This means the ID will not randomly change each time you plug in the cable.

The approach I use is to first open the Device Manager and search for the Ports (COM & LPT). If the node is not in the list, everything is fine. It the node is part of the list, unfold the node and check all existing COM ports and memorize the ids. Then plug in the cable to your new device. Windows will detect the serial connection, dynamically create a new COM port for your device and the Device Manager will reload, displaying the new COM port. If you have remembered the old list, you are now able to identify the new id of the new COM port added. Use this id in a terminal emulator such as YAT to establish a connection using the baudrate of 19200 since this is the baudrate that is configured in the NEORV32 source code.

In the case of the NEORV32 CPU, the CPU provides a UART for a human to connect their Laptop/Desktop PC to and to start a terminal emulator that connects to a COM port on windows. Using that UART it is possible to interact with the bootloader.
...
cd /c/Users/lapto/dev/VHDL/neorv32-setups/neorv32/bootloader
...
The neorv32_exe.bin file contains data that the bootloader can process. The data is generated by the sw\image_gen\image_gen.c application from the application binary file. The application binary file contains the applications raw machine code. That way on the FPGA the Bootloader only has to copy those bytes into RAM and it does not has to understand complicated file formats such as intel hex or even then .elf file format.

The applications binary data is taken from the build/main.bin file as we can see from the console output of the makefile:

```
Generating neorv32_exe.bin
../../../sw/image_gen/image_gen.exe -t app_bin -i build/main.bin -o neorv32_exe.bin
```

The build/main.bin file is created from the .elf file using the object dump tool:

```
riscv-none-elf-objcopy -I elf32-little main.elf -j .text -j .rodata -j .data -O binary build/main.bin
```

This is a bit convoluted but it is easy to understand if you memorize that the GCC compiler will output a .elf file. .elf is the executable and linkable file format used on many Unix flavored operating systems. On windows, the equivalent would be the PE file format. The GNU compiler tool suite contains many tools to manage .elf files out of which the objcopy file allows us to extract the code from a .elf file into a binary blob. That binary blob contains machine code without meta-data such as the .elf file contained originally. The goal is to transfer this binary blob to the bootloader over UART.

image_gen is called by the makefile of every example. In order to create the file that the bootloader will accept, image_gen adds a header to the application binary file, which is the blob described above.

The header consists of twelve bytes and stores three values in total within these twelve bytes. It stores a four byte signature. This signature or magic word as it is describe inside the image_gen source code is the fixed byte sequence:
...
Following the signature is a four byte field that stores the size of the data that follows after the header. Size does not include the header size itself!

The last field of the header is a four byte field storing a checksum. The checksum is computed by starting with the value zero and then by continuously adding up the entire application data file in 4 byte values to the checksum. Finally the sum is inverted.

```
checksum = ~checksum;
```

The tilde operator turns a zero bit into a 1 bit a vice versa. This operation is performed for each bit in the number.

This means that the checksum is not just the accumlated sum of the machine code, but it is the inverted accumulated sum.

While overflows during adding are ignored, the checksum is not actually a mathematically valid sum but it does not have to be either! It just is used as a cross check which can be computed on the sender and also on the receiver side.

Unless we are extremely unlucky, if there is an error during transmission, the checksum will not compute the same way on the receiver's side as it does on the sender'S side. If the bootloader computes the checksum, it can compare the locally computed checksum by the sender's checksum to see if the upload was completed without missing or garbled bytes. If the checksum does not check out, then the bootloader will not load and boot the transmitted software.

The CRC is not used to encrypt or repair the message it is much rather used to decide weather to trust the message or not. The CRC is similar to wax seals applied to letters back in the day. If someone broke the wax seal, this could be easily detected by the receiver. To melt and reapply a new seal, a perpetrator had to be in possesion of the sender's official wax-seal-stamp which should not ever be the case ever. The sender should never hand out or get their stamp stolen! A letter with a broken or unexpected seal is simply ignored by the receiver as it has been tampered with or read by someone else.
...
When comparing two sequences of code, one important metric is the amount of cycles it takes the CPU to process the instructions in each sequence. The sequence with less amount of cycles consumed is then deemed either better or worse then the other. Basically this mapping places code on a scale which allows code to be compared.

The NEORV32 CPU allows you to perform this mapping. Let's look at code from the neorv32\sw\example\demo_cfu\main.c example.
...
```
  ...
  uint32_t time_enc_sw = neorv32_cpu_csr_read(CSR_MCYCLE); // stop timing
...
A CSR is a configuration and status register. Different RISC-V Extensions mandate certain CSR registers. CSR registers are not used to perform computation within the RISC-V core but they are used to inspect or even set configuration parameters that may affect the CPUs state which then indirectly might have an effect on the execution of application software.

The vector extension needs a certain bit set in on of it's CSRs before it even starts to operate. Without the bit set, it will throw an error.

The CSR_MCYCLE CSR used in the example above is documented in the NEROV32 datasheet (https://stnolting.github.io/neorv32/#_machine_counter_and_timer_csrs).

```
0xb00, mcycle, CSR_MCYCLE, MRW, Machine cycle counter low word
0xb80, mcycleh, CSR_MCYCLEH, MRW, Machine cycle counter high word
```

This means, there is a CSR for the low word and another one for the high word of the cycle counter! If the expected cycles are only a few, then it is enough to only look at the low counter as the amount of cycles will not spill into the high word. This approach is used in the example above.

What does machine cycle counter mean exactly? The description in the datasheet says:
...
The important part is "will increment with every active CPU clock cycle". My interpretation of this sentence is that as long as the core is not put to sleep, every external clock source clock tick will be measured by also incrementing CSR_MCYCLE / CSR_MCYCLEH. So when measuring CSR_MCYCLE / CSR_MCYCLEH the actual measured value is the clock ticks of the external system clock that makes the CPU step forward.

This means that using CSR_MCYCLE / CSR_MCYCLEH the mapping will not map to executed instructions but to actual clock ticks passed.
